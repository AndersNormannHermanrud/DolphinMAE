...
def debug_tensor_stats(name, tensor):
    ...
    if torch.isnan(tensor).any():
        print(f"[NaN] {name} contains NaNs")
    if torch.isinf(tensor).any():
        print(f"[Inf] {name} contains Infs")
    #print(f"[Stats] {name}: min={tensor.min().item():.4e}, max={tensor.max().item():.4e}, mean={tensor.mean().item():.4e}, std={tensor.std().item():.4e}")
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio, mask_2d=False):
        ...
        #Debug
        debug_tensor_stats("Encoder output x", x)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        # Debug
        debug_tensor_stats("Decoder norm x before pred", x)

        # predictor projection
        pred = self.decoder_pred(x)

        debug_tensor_stats("Decoder pred", pred)
    ...
    def forward_loss(self, imgs, pred, mask, norm_pix_loss=False):
        ...
        #show_np_spectrogram_file(imgs[0,0,:,:].cpu().numpy())
        #print("Why am i not running")

        debug_tensor_stats("Patchified target", target)
        debug_tensor_stats("Pred before loss", pred)
        ...
        debug_tensor_stats("Patch loss", loss)
        return loss
    ...
    def forward(self, imgs, mask_ratio=0.8):
        ...
        debug_tensor_stats("Final loss", loss_recon)

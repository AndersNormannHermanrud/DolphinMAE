...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
        ...
        # use a pretrained models for finetuning
            ...
            #sd = torch.nn.DataParallel(sd) #If you use ssast owm pretrained checkpoint, uncomments this, this fixes own checkpoints
    ...
    def mpc(self, x, mask_patch, cluster, show_mask=False):
        ...
        #Debug code to detect silent Nan propagation, slows down code
        #idx = mask_index[i].item()
        #if idx + self.cls_token_num >= x.shape[1]:
        #    print(f"mask index[i] {idx}, token num {self.cls_token_num}, xshape[1] {x.shape[1]}")
        #    raise RuntimeError(f"Index out of bounds: mask_index + cls_token_num = {idx + self.cls_token_num}, x.shape[1] = {x.shape[1]}")
        #if torch.isnan(x[i, idx + self.cls_token_num, :]).any():
        #    print(x[i, idx + self.cls_token_num, :])
        #    raise RuntimeError("NaN before feeding to cpredlayer")
        ...
        self.mask_correct = torch.nn.Parameter(torch.arange(0, mask_patch), requires_grad=False).to(x.device)
        ...
        for i in range(B):
            ...
            pred[i, mask_index[i], :] = torch.tensor(result).reshape(mask_patch, 1).expand(mask_patch, 256).to(x.device)
    ...
    def mpg(self, input, mask_patch, cluster, show_mask=False):
        ...
        pred_unfolded = input.clone()
        masked_unfolded = input.clone()

        for i in range(B):
            pred_unfolded[i, mask_index[i]] = pred[i]  # fill with model's predicted pixels
            masked_unfolded[i, mask_index[i]] = 99.0  # fill with debug constant
        ...
        pred_4d = fold(pred_unfolded.transpose(1, 2))  # => [B,1,freq,time]
        masked_4d = fold(masked_unfolded.transpose(1, 2))  # => [B,1,freq,time]

        return pred_4d, masked_4d
    ...
    def forward(self, x, task, cluster=True, mask_patch=400):
        ...
        elif task == 'visualize_mask': #not changed to visualize_mask_mpc for legacy reasons
            return self.mpc(x, mask_patch=mask_patch, cluster=cluster, show_mask=True)
        elif task == 'visualize_mask_mpg':
